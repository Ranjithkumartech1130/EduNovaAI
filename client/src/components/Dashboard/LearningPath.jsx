        ...
        <div className="tab-path glass-card">
            ...
            {pathResult && (
                <div ref={resultRef} className="lp-results animate-fade-in">
                    ...
                    {/* Visual Roadmap */}
                    <div className="lp-section lp-roadmap-section">
                        <div
                            className="lp-section-header"
                            onClick={() => toggleSection('roadmap')}
                        >
                            <h5 className="lp-section-title">
                                <Map size={20} className="lp-section-icon" />
                                <span>Visual Roadmap</span>
                            </h5>
                            {expandedSections.roadmap === false ? <ChevronDown size={18} /> : <ChevronUp size={18} />}
                        </div>

                        {expandedSections.roadmap !== false && (
                            <>
                                <div className="lp-roadmap-container">
                                    <img
                                        src={`http://localhost:8002/generate-flowchart?goal=${encodeURIComponent(goal || 'Learning Path')}&t=${Date.now()}`}
                                        alt="Roadmap Flowchart"
                                        className="lp-roadmap-img"
                                    />
                                </div>
                                <div className="lp-roadmap-actions">
                                    <button className="lp-action-btn"><FileText size={14} /> Export TXT</button>
                                    <button className="lp-action-btn"><ImageIcon size={14} /> Export PNG</button>
                                </div>
                            </>
                        )}
                    </div>
